...
class HomePage:
    ...
    def words_button_click(self):
        global n
        root.destroy()
        n = simpledialog.askinteger("Enter n", "Enter the number of letters in the word:")
        if n is not None:
            self.wordle = tk.Tk()
            self.wordle.title("Wordle with Bulls and Cows")
            game = Wordle(self.wordle)
            self.wordle.mainloop()
class Wordle:
    def __init__(self, master):
        global n
        self.master = master
    ...
    def generate_secret_code(self):
        global n
        return ''.join(random.sample('qwertyuiopasdfghjklzxcvbnm', n))

    def check_guess(self):
        global n
        guess = self.guess_entry.get()
        if len(guess) != n:
            messagebox.showerror("Error", f"Please enter a valid {n}-letter word.")
            return
